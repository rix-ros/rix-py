...
def install(model_dir: str) -> None:
    ...
    json_files = [f for f in os.listdir(model_dir) if f.endswith(".json") and f != "model.json"]
    if len(json_files) == 0:
        print(f"Error! No JRDF file found in '{model_dir}'.")
    
    try:
        processor = JsonMacroProcessor(merge_strategy=JsonMergeStrategy.MERGE_WITH_ERROR)
    ...
    properties["$comment"] = "This file was generated by jrdf from jsonmacros. Do not edit directly."
    ...
    try:
        ...
        if os.path.islink(dest_dir) or os.path.isdir(dest_dir):
            print(f"Model '{model_name}' is already installed. Uninstall it first.")
            return
    except Exception as e:
